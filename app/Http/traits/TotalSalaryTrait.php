...
Trait TotalSalaryTrait {
	...
	// 	return $total;
	// }
	public function totalSalary($employee, $payslip_type , $basic_salary, $total_minutes =1){

		if($payslip_type == 'Monthly')
		{
			$total = $basic_salary + $employee->allowances->sum('allowance_amount') + $employee->commissions->sum('commission_amount')
				- $employee->loans->sum('monthly_payable') - $employee->deductions->sum('deduction_amount')
				+ $employee->otherPayments->sum('other_payment_amount') + $employee->overtimes->sum('overtime_amount');
		}
		else
		{
			...
			$total =  ($basic_salary / 60) * $total_minutes + $employee->allowances->sum('allowance_amount') + $employee->commissions->sum('commission_amount')
				- $employee->loans->sum('monthly_payable') - $employee->deductions->sum('deduction_amount')
				+ $employee->otherPayments->sum('other_payment_amount') + $employee->overtimes->sum('overtime_amount');
		}
		return $total;
